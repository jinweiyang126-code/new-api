...
后台 / 组织侧**资源列表页**（用户、客户、工作区、上游凭证等）须对齐 **Users** 页的布局与交互，不得退回为页面内嵌表单 + 卡片列表。

- **完整标准**：[docs/design/admin-list-page-standard.md](../docs/design/admin-list-page-standard.md)
- **金标准实现**：`src/features/users/`
- **必须具备**：`SectionPageLayout`、列排序、多选、批量栏壳、搜索/筛选/分页（URL）、操作栏标题 `Actions`、编辑笔（打开编辑抽屉）、行菜单、创建抽屉、危险操作确认框；详情只读、编辑可写
...
- **2026-08-14**：新增 3.3.1 管理端列表页标准（对齐 Users DataTable；详见 `docs/design/admin-list-page-standard.md`）。
