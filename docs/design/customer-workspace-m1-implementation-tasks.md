...
> 范围：**M1** = 组织 / 工作区 / 成员 / 邀请 / 额度划拨 / 工作区池扣费 / 隔离 / **独立上游·BYOK**  
> 术语：组织=Customer，工作区=Workspace（旧称企业/项目已废弃）  
> 不含：机构层、工作区模型白名单（M3）、支付直充组织池、SSO（M5）  
> 自助开户（注册页个人/组织）：见 [`customer-self-register-design.md`](./customer-self-register-design.md)，不在本 M1 任务表内 

**实现约定（设计开放项按建议落地，避免阻塞）：**

1. 用户退出/被移出组织后，**允许**再加入另一家组织  
2. 普通成员只能管理/看到**自己的**工作区令牌；组织/工作区 admin 可见本范围内全部令牌  
3. 组织池充值一期仅 **平台超管 API/后台操作**，不做在线支付  
4. 新组织默认 `upstream_mode=shared`；BYOK/专属渠道由超管按组织开通  
...
| --- | --- | --- | --- |
| **门禁 1** | **T08** 工作区池扣费 | T01–T07；T09 日志可并行草稿 | **不得**开始依赖「组织令牌已正确扣费」的后续验收；**不得**进入 T14/T15 联调（可先写 T14 API，但不得宣称计费闭环完成） |
            ...
            → T07 令牌扩展
              ...
              → T14 BYOK/独立上游 API + Key 加密   ← 依赖 T01/T02/T03；联调依赖 T08 已稳
                → ★ T15 Relay 选渠  【硬门禁 2：测稳再继续】← 依赖 T08+T14
                  → T10 前端平台超管「组织管理」+ 上游设置
                    → T11 前端组织侧（工作区/成员/邀请/额度/BYOK 凭证）
...
| T02 | 组织/工作区权限与请求上下文 | 1.5 | P0 |
| T03 | 组织 API（含创建事务、充值） | 1.5 | P0 |
| T04 | 工作区 API（含划拨事务） | 1 | P0 |
| T05 | 组织/工作区成员 API | 1 | P0 |
| T06 | 邀请创建/接受/作废 | 2 | P0 |
...
- [x] 单测或迁移烟测覆盖唯一索引（同组织下 workspace slug 不重复；binding 唯一）
- [x] 新建组织默认 `upstream_mode=shared` — *常量 `UpstreamModeShared`；创建时显式写入（T03 事务）*
...
1. 定义角色常量：`owner` / `admin` / `member`（组织与工作区）  
2. 辅助函数：  
...
- [x] 非成员访问其他组织 API 返回 403 — *middleware `CustomerMemberAuth` + 单测*
- [x] 超管可跨组织操作 — *`IsRootUser` bypass + 单测*
...
| 接口 | 说明 |
| --- | --- |
| `GET /api/customers` | 超管：全部；组织用户：仅自己的组织（或 403 非超管只返回当前） |
| `POST /api/customers` | **仅超管**；事务：组织 + default 工作区 + owner 成员 + `users.customer_id` |
| `GET /api/customers/:id` | 详情含额度 |
| `PUT /api/customers/:id` | 改名/备注/停用（超管；组织 admin 是否可改名：建议仅超管停用，admin 可改名——实现时按「超管全权，owner/admin 可改 name/remark」） |
| `POST /api/customers/:id/topup` | **仅超管**；增加 `customers.quota`；写系统/审计日志 |
| `GET /api/customers/:id/workspaces` | 成员可读本组织工作区列表 |

创建组织事务失败条件：`owner` 用户已有 `customer_id`。

### 验收

- [x] 创建后存在 `slug=default` 且 `is_default=true` 的工作区 — *`TestCreateCustomerWithOwnerCreatesDefaultWorkspace`*
- [x] owner 的 `users.customer_id` 已设置 — *同上；已有组织再创建失败*
...
| 接口 | 说明 |
| --- | --- |
| `POST /api/customers/:id/workspaces` | 组织 admin+ |
| `GET /api/workspaces/:id` | 工作区成员可读 |
| `PUT /api/workspaces/:id` | 更新/停用 |
| `POST /api/workspaces/:id/transfer-quota` | 组织 admin+；事务划拨 |
...
| `DELETE /api/customers/:id/members/:userId` | 不可移除最后一位 owner；清理 `users.customer_id`；**禁用**该用户在该组织下所有令牌 |
| `GET/POST/DELETE /api/workspaces/:id/members` | 加入工作区的用户必须已是组织成员 |

### 验收

- [x] 移除成员后无法用旧组织令牌调用（status 禁用） — *`TestRemoveCustomerMemberDisablesTokensAndClearsCustomerId`（个人令牌不受影响）*
- [x] 非组织成员不能加入工作区 — *`TestAddWorkspaceMemberRequiresCustomerMembership`*
- [x] 跨组织操作 403 — *`TestCustomerMemberAuthCrossCustomerForbidden`*；另覆盖不可移除最后一位 owner
...
接受逻辑按设计文档第 10 节；已有组织则明确错误码/文案。

**邮件（T06b）：** 若 SMTP 已配置且提供 email，发送邀请链接；失败不阻断邀请记录创建。链接格式：`{ServerAddress}/invitations/accept?token=...`。创建接口返回 `email_sent` / `email_error`。

### 验收

- [x] 无组织用户可接受并进入组织 + 工作区 — *`TestAcceptInvitationJoinsDefaultWorkspace`*
- [x] 已有组织用户接受失败 — *`TestAcceptInvitationFailsWhenUserAlreadyHasCustomer`*
...
1. `POST /api/token`：可选 `workspace_id`  
   - 有：校验工作区成员；写入 `customer_id`/`workspace_id`；**令牌额度字段**可与现网 remain_quota 并存，但 **Relay 扣费以工作区池为准**（令牌 remain 可作为附加上限，M1 建议：组织令牌仍可设 remain_quota，扣费时两者都检查——先简单：**只扣工作区池**，令牌 remain_quota 对组织令牌可忽略或同步减少，实现时在任务内写清并单测）  
2. `GET /api/token`：按成员过滤；admin 看范围全部  
3. 创建组织令牌时工作区/组织必须启用  

**M1 扣费建议（写进实现注释）：**  
组织令牌（`workspace_id != null`）：**仅扣 `workspaces.quota`**；不扣 `users.quota`；令牌 `remain_quota` 若 `unlimited=false` 则同时作为次级上限（与现网一致），避免一张令牌掏空工作区——**推荐采用「工作区池 + 令牌 remain 双限制」**。
...
> **停下来测稳**：本任务验收未全部通过前，不要进入「组织令牌计费已闭环」的后续结论，也不要开始 T15 选渠联调。
...
4. 组织或工作区停用：拒绝转发  
5. 渠道选择：本任务对 `shared` 组织保持现网逻辑；**dedicated/byok/hybrid 在 T15 接入**（T08 可先只保证 shared）  

重点排查现有：`service/quota.go`、`service/text_quota.go`、预扣消费、退款、订阅路径——**组织令牌一期不走订阅抵扣也可，但必须明确；建议组织令牌仅工作区池，不碰订阅**（若现网强耦合，在提交说明里写清例外）。

### 验收（全部勾选 = 门禁 1 放开）

- [x] 组织令牌调用后：工作区池减少，用户个人余额不变 — *`TestWorkspaceBillingDebitsPoolNotUser` / `TestWorkspaceBillingSettleTopUp`*
...
3. ~~组织 admin+：upstream-credentials CRUD；明文 key 仅写入时接收，落库加密；列表仅 hint~~ — *`common.EncryptSecretAESGCM` + credential ops/API*
...
- [x] shared 组织与现网选渠一致 — *`TestSelectChannelSharedCustomerUsesGlobalPath`*
- [x] dedicated 仅走绑定渠道（除非 fallback） — *`TestSelectChannelDedicatedOnlyBoundChannel` / `NoFallbackErrors`*
- [x] BYOK 实际使用组织 Key（测试 upstream） — *`TestSelectChannelByokUsesCustomerKey`（组装临时 Channel，Key 解密仅内存）；真实出站复跑见 T13「验收复跑记录」`scripts/byok-live-smoke.mjs`*
- [x] 个人令牌不受组织 BYOK 影响 — *`TestPersonalTokenAlwaysSharedSource`*
...
- [x] A 组织 admin 拉不到 B 组织日志 — *`TestCustomerAdminCannotSeeOtherCustomerLogs` / `TestResolveSelfLogAccessScopeCustomerAdminIsolation`（伪造 `customer_id` → empty）*
- [x] 组织令牌产生的日志带齐组织/工作区字段 — *`TestRecordConsumeLogWritesCustomerWorkspace` / context fallback*
...
- [x] 超管可完成开组织、充值 — *平台菜单「组织管理」+ Create / Top Up；Playwright 浏览器烟测通过*
- [x] 创建后详情可见 default 工作区与余额 — *详情抽屉拉取 `/workspaces`；浏览器可见 default*
- [x] 超管可为组织开通 BYOK/专属渠道并保存 — *`PUT upstream-settings` + channel-bindings（T14 最小切片）；浏览器保存 dedicated + BYOK + 绑定渠道通过*

---

## T11 — 前端：组织侧工作区 / 成员 / 邀请 / 额度 / BYOK
...
- [x] 组织 admin 全流程可邀人、划拨 — *Organization 菜单 + workspaces/members/quota/upstream；API 烟测 owner7 invite + transfer-quota 通过*  
- [x] 已有组织用户接受邀请有明确失败提示 — *`/invitations/accept` 对 `user already belongs to a customer` 专用 toast（en/zh）*
...
2. 令牌列表按权限过滤  
3. 顶栏：组织名（只读）+ 当前工作区切换（调用 `current-workspace`，影响默认创建令牌的工作区预填）  
...
1. 后端单测：创建组织事务、划拨、邀请、扣费分支、日志过滤、**选渠 shared/dedicated/byok**、Key 不落日志  
2. 手工/脚本 E2E 清单（见下）  
3. 更新 `customer-workspace-design.md` §15 实现状态为「开发中/已完成」  
4. （可选）在 `aliyun-ecs.md` 或运维备忘中注明：组织功能无需额外端口  
...
- [x] §15 实现状态已更新  
- [x] 运维备忘：组织/工作区功能无额外端口  

### E2E 清单

- [x] 超管创建组织 A、B，各充值 — *`scripts/t13-e2e.mjs` `e2e-create-topup`*  
...
- [x] 个人用户（无组织）原有 curl 流程仍通 — *`e2e-personal-user`*  
- [x] **BYOK**：为 A 开通 byok，配置测试 Key，工作区令牌走组织 Key；关闭后回落 shared（若 fallback） — *API `e2e-byok-dedicated-api`；选渠 `TestSelectChannelByok*`；fallback `TestSelectChannelByokDisabledIgnoresCredentials`；**真实出站**见下方复跑记录*  
...
| `node scripts/byok-live-smoke.mjs` | **4 PASS / 0 FAIL** | BYOK 真实出站冒烟：本地 OpenAI 兼容 mock（`host.docker.internal:18081`）；`upstream_mode=byok` + `allow_global_fallback=false`；工作区令牌 `/v1/chat/completions` 成功；mock 收到的 `Authorization` 为组织 BYOK Key；消费日志 `upstream_source=byok` |
...
| 里程碑 | 包含任务 | 可演示结果 | 门禁 |
| --- | --- | --- | --- |
| M1-a 骨架 | T01–T04 | 超管开组织、充值、划拨 | — |
| M1-b 协作 | T05–T06 | 邀请进组织 | — |
