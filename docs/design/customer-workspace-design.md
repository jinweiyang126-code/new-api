...
1. **多组织隔离**：不同组织的成员、令牌、日志、额度互不可见  
2. **工作区**：组织下的工作区承载成员与 API 令牌  
3. **额度编排**：平台给组织充值 → 组织划拨到工作区 → 调用只扣工作区池  
4. **邀请制加入**：用户先注册个人，再由组织管理员邀请进组织  
5. **上游灵活**：默认全球共享；M1 即支持 **独立上游绑定 + BYOK**（按组织开通，默认关）  
6. **兼容现网**：未加入组织的个人模式保持现有行为  

### 1.2 非目标（当前不做）

- **机构层**（集团 / 代理商再挂多家组织）— 明确后置，先不考虑  
- 用户同时属于多家组织  
- 组织流量扣减 `User.Quota`（个人余额）  
- 组织自助注册开户 — **已另文**：[customer-self-register-design.md](./customer-self-register-design.md)；超管开户仍保留
- 在线支付直接充入组织池（一期可用超管充值；支付合规为产品外事项）  
...
| 项 | 结论 |
| --- | --- |
| 命名 | **组织**、**工作区**（英文：Customer / Workspace） |
| 组织层级 | 平台 → 组织 → 工作区（**无机构层**） |
| 组织数量 | 不设上限 |
| 用户 ↔ 组织 | **一对一**：同一时刻不可属于多家组织 |
| 注册加入 | 默认个人注册；可选组织自助开户（见自助开户文）；也可 **邀请进组织** |
| 邀请权限 | **组织管理员可邀请成员**（超管可代管） |
| 计费 | 组织令牌 **只扣工作区池**，**完全不碰** `User.Quota` |
| 渠道 / BYOK | **M1 正式范围**：默认 `shared`；可按组织开通独立上游 / BYOK（设计见 **§16**） |
| 默认工作区 | 创建组织时 **自动创建** `default` 工作区 |
| 个人模式 | `customer_id` 为空：个人令牌仍走个人余额；**永不走组织 BYOK** |
...
平台超管
 └── 组织 Customer
      ├── 组织管理员（owner / admin，可邀请成员）
      ├── 组织额度池（平台充值）
      └── 工作区 Workspace（创建组织时自动建 default）
           ├── 成员
           ├── 工作区额度池（API 扣费账户）
           ├── API 令牌
           └── （后续）模型白名单等策略
...
| 工作区 | 工作区 | 组织内工作区/环境 |
| 工作区额度 | 工作区配额 | 实际扣费账户 |
| 平台渠道 + BYOK | 网关上游 | 默认共享；M1 可按组织专属/自带 Key |

### 3.2 渠道与 BYOK（M1）

- **默认**：上游渠道由平台超管统一配置，组织未开通时全部走全球共享（与现网一致）。  
- **M1 已包含**：按组织开通 **独立上游绑定** 与/或 **BYOK**；完整数据/选渠/接口/安全见本文 **§16**。  
- 隔离的是：成员、令牌、日志、额度；以及（开通后）该组织可用的上游候选集。  
- 个人令牌始终只走全球共享。
...
| 能力 | 平台超管 | 组织 owner/admin | 工作区 admin | 成员 |
| --- | --- | --- | --- | --- |
| 创建/停用组织 | ✓ | | | |
| 给组织充值 | ✓ | | | |
| 创建工作区 | ✓ | ✓ | | |
...
1. 组织管理员创建邀请（邮箱和/或链接 token；可指定默认工作区与角色）  
2. 被邀请人：  
   - 未注册 → 注册后接受邀请加入该组织  
   - 已注册且无组织 → 接受邀请加入  
   - 已注册且已有组织 → **拒绝**（提示已属于其他组织）  
3. 接受后加入组织成员，并加入指定工作区或 `default` 工作区  
...
- 增加可空 `customer_id`：空=个人模式；非空=已入该组织（一对一）

**`tokens`**

- 增加可空 `customer_id`、`workspace_id`  
- `workspace_id` 非空时组织令牌，扣工作区池；为空则个人令牌  
...
| POST | `/api/customers/:id/topup` | 组织充值 |
...
| POST | `/api/workspaces/:id/transfer-quota` | 从组织池划拨到工作区 |
...
| GET | `/api/log/...` | 按组织/工作区/本人范围过滤 |
| GET | `/api/user/self/customer` | 当前组织、工作区、角色 |
...
3. 写日志带上组织/工作区 ID；若走专属/BYOK 则标记 `upstream_source`（**不写 Key**）  
4. **选渠**：按组织 `upstream_mode` 执行 §16.5 顺序（默认 `shared` = 现网全球选渠）  
...
BeginTx
  ...
  update users.customer_id   -- owner 若已有组织则失败
Commit
```
...
若 users.customer_id 已有且不是本组织 → 错误
BeginTx
...
禁用该用户在该组织下所有工作区令牌，并清空 `users.customer_id`（若其仅属于该组织）。
...
| 阶段 | 内容 |
| --- | --- |
| **M1** | 组织/工作区/成员/邀请；充值与划拨；工作区池扣费；隔离；**独立上游/BYOK** |
| **M2** | 使用账本、导出、组织分群、预算预警（对照 DogRouter「使用账本 / 供应商报表」） |
| **M3** | 已审批模型白名单、工作区/令牌限流（对照「策略 → 已审批流量」） |
| **M4** | 组织在线充值、流水/授信/开票（轻量）、成本对账（轻量） |
...
若未来需要机构，见 **§B.9.7** 单独立项草案；M1 **不预埋**机构表。组织表若日后扩展，可再增加可空 `institution_id`。
...
1. 用户退出组织后，是否允许加入另一家（建议：允许，因仍满足「同一时刻只属一家」）  
2. 组织令牌是否允许成员看到同工作区他人令牌（建议：默认仅自己的，admin 可见全部）  
3. 组织池充值：M1 先超管线下；在线支付见 M4  
...
> **状态**：设计已确认，**纳入 M1 交付**。  
> **默认**：新组织 `upstream_mode=shared`（全球共享，与现网一致）。  
> **开通**：超管按组织打开 dedicated / byok / hybrid；商业上可作为加购开关，工程上属 M1。  
...
| 模式 | 含义 | 谁提供上游 Key |
| --- | --- | --- |
| **A. 独立上游（专属渠道绑定）** | 该组织令牌优先/仅走平台为其配置的渠道 | 平台（超管建渠道并绑定组织） |
| **B. BYOK** | 组织上传自有上游 Key，请求用该 Key 出站 | 组织 |

未开通时：行为与 M1 完全一致（只走全球共享渠道）。

### 16.2 非目标

- 组织在 UI 里随意创建任意类型上游（一期：超管开通 + 组织仅管理自己的 BYOK 凭证）  
...
| priority | 越小越优先 |
| model_mapping | 可选 JSON；覆盖该组织模型名映射 |
| status | 启用/停用 |
| created_at / updated_at | |
...
### 16.5 Relay 选渠顺序（组织令牌）

```text
1. 令牌校验、组织/工作区启用、白名单、工作区池、限流（既有）
...
| **上游成本** | BYOK 组织在 M4d 对账中供应商成本记 0 或 N/A | 与 M4d 联动 |
...
| GET | `/api/customers/:id/upstream-credentials` | 组织 admin+ | 列表（仅 hint，无密文） |
| POST | `/api/customers/:id/upstream-credentials` | 组织 admin+ | 创建；body 含明文 key，服务端加密后丢弃明文 |
| PUT | `/api/customers/:id/upstream-credentials/:cid` | 组织 admin+ | 轮换 key / 改 models |
| DELETE | `/api/customers/:id/upstream-credentials/:cid` | 组织 admin+ | 删除 |
| POST | `/api/customers/:id/upstream-credentials/:cid/test` | 组织 admin+ | 探测连通（可选） |

所有写操作写审计日志（actor、customer_id、credential_id、动作；**无 Key**）。

### 16.8 UI

- 超管「组织详情」：上游模式、绑定渠道、是否允许回落、开通 BYOK  
- 组织后台「上游 / BYOK」：凭证列表、添加/轮换/删除、测试（若实现）  
- 文档提示：Key 泄露责任在组织；平台仅托管加密存储  

### 16.9 安全

1. 落库必须加密；进程内存解密窗口尽量短  
2. API 永不返回完整 Key；导出日志脱敏  
3. 权限：仅组织 admin/owner 与超管可管理；普通成员不可见 hint 以外信息（或连列表都不可见，可配置）  
4. 删除组织或关闭 byok_enabled：停用凭证，既有请求不再使用  

### 16.10 验收

- [ ] `upstream_mode=shared` 的组织与 M1 选渠结果一致  
- [ ] dedicated 绑定后，该组织流量不进入未绑定的共享渠道（除非 fallback）  
- [ ] BYOK 请求实际使用组织 Key（可用测试 upstream 验证）  
- [ ] 接口/日志/审计均无完整 Key  
- [ ] 个人令牌不受组织 BYOK 配置影响  
- [ ] 工作区池扣费仍生效（按计费策略）  
...
| 阶段 | 关系 |
| --- | --- |
| M1 | **含 BYOK**；`shared` 组织选渠与现网一致 |
| M3 | 白名单仍在 BYOK 之前校验 |
| M4d | BYOK 组织成本侧标记为组织自付 |
| M5 | 可与 SSO 同属加购包对外售卖 |
...
DogRouter 组织页公开能力可概括为四块：

| # | DogRouter 能力 | 我们现状 / M1 | 本文设计归属 |
| --- | --- | --- | --- |
| A | 组织 / 工作区 + 配额编排 | M1：组织 / 工作区 + 工作区池扣费 | ✅ M1 已覆盖 |
| B | 使用账本、导出、组织分群 | 仅有通用日志 | **M2** |
| C | 策略治理：已审批模型、限流、审批阈值 | 令牌模型限制较弱 | **M3** |
| D | 组织计费：预付进池、开票、信用、税务、对账 | 超管线下充值 | **M4** |
| E | 组织加固：审计增强、SSO、私有化策略、SLA | 基础审计/2FA | **M5** |
...
2. **M1 含独立上游 / BYOK**（§16）：默认全球共享；按组织开通 dedicated/byok/hybrid。  
3. **计费主账户仍是工作区池**；组织池、信用额度只做分配与授信，不改变「调用扣工作区」主路径。  
4. **个人模式继续隔离**：下列能力默认仅对组织令牌 / 组织后台生效。  
...
- 「使用账本：跨组织、模型、团队与计费主体追踪支出与 Token」  
- 「供应商报表：使用导出、流量摘要与组织分群」  
- 「按工作区、环境、用户维度追踪消耗」

### 2.2 目标

1. 组织管理员可按 **时间 / 工作区 / 成员 / 模型** 查看用量与费用汇总  
2. 支持 **CSV 导出**（平台超管可跨组织导出）  
3. 基础「组织分群」：按组织标签或消耗层级筛选（轻量）  
4. 可选：工作区/组织预算 **预警**（达 80%/100% 通知，不替代硬限额）
...
- 组织后台新增 **用量 / 账本**  
- 筛选器 + 汇总表 + 导出按钮  
- 超管组织管理列表支持按消耗/标签排序筛选  

### 2.7 验收

- [ ] 组织 A 看不到组织 B 汇总  
- [ ] 导出字段含组织、工作区、用户、模型、额度、时间范围  
- [ ] 个人模式用户无此菜单（或仅个人日志，不叫组织账本）  
...
2. **组织级默认策略**可下发到新工作区（可选覆盖）  
3. **工作区/令牌级速率限制**（RPM/RPD 或沿用现网模型限流并挂到工作区维度）  
4. （可选增强）模型变更需组织 admin **审批** 才进白名单——M3 可先做「管理员直接编辑白名单」，审批流见 **§9.1 M3.1**  
...
- 「组织计费：预付费余额、开票、信用额度、税务文档与对账」  
- 「合并计费、受控使用」  
- 「将供应商发票与组织使用账本、付款、积分与调整项匹配」

### 4.2 目标（可拆两刀）

**M4a 资金入组织池（在线）**

1. 支持支付成功后贷记 **组织池**（不是个人钱包）  
2. 组织 admin 发起充值订单；平台配置收款方式（复用现有支付网关能力）  
3. 完整订单状态机：创建 → 支付中 → 成功入账 / 失败  

**M4b 商务单据与授信（按需）**

1. **信用额度** `customers.credit_quota`：工作区可支用上限 = 现金池 + 信用（或仅允许组织池透支到信用线）  
2. **调账**（平台超管）：加减组织池并记原因  
3. **对账单 / 发票申请**：导出周期账单；发票信息由组织维护，开票流程可先半人工  
4. **上游成本对账（轻量）**：按日汇总「对组织收费额度」vs「渠道成本估算」（依赖渠道成本配置，允许误差）  
...
| 表 | 说明 |
| --- | --- |
| `customer_topup_orders` | 组织充值订单：金额、支付渠道、状态、customer_id |
| `customer_ledger_entries` | 组织流水：topup / transfer_out / transfer_in / adjust / credit |
...
组织可划拨余额 = customers.quota + 剩余可用信用
调用仍只扣 workspace.quota
信用仅影响「组织能划拨/预支多少到工作区」，不直接在 Relay 扣信用
```

### 4.5 合规注意（设计约束）

- 上线 M4a 前需完成产品外合规评估（经营主体、支付资质、合同）。  
- 技术侧：支付回调必须校验组织 ID，**禁止**写入个人 `User.Quota`。  
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| POST | `/api/customers/:id/topup-orders` | 组织 admin 创建支付订单 |
| GET | `/api/customers/:id/ledger` | 组织流水分页 |
...
| 能力包 | 内容 |
| --- | --- |
| **M5-SSO** | 组织级 OIDC/SAML 登录；成员仅能通过组织 IdP 进入（可与现有 OIDC 配置按 customer 绑定） |
...
- [ ] 强制 SSO 开启后，密码登录对组织成员关闭（超管豁免策略另定）  
- [ ] 关键策略/划拨/导出均有审计记录且组织隔离  
...
```text
【主路线 — 已确认】
M1   组织/工作区/邀请/工作区池扣费/隔离 + **独立上游/BYOK**（默认 shared）
M2   使用账本、导出、分群、预算预警
M3   已审批模型白名单、工作区限流
M4   组织在线充值 + 流水/授信/开票（轻）+ 成本对账（轻）
...
**不因「商务催收款」默认插队 M4**。BYOK **已在 M1**：未开通组织行为等同全球共享。
...
| 3 | M4 信用语义 | 信用只影响 **组织可划拨上限**；**调用仍只扣工作区池** |
| 4 | M5 SSO / 私有化策略等 | **商务加购**，默认关闭 |
| 5 | 机构层 | **主路线不做**；设计见 **§B.9.7** |
| 6 | 独立上游 / BYOK | **纳入 M1 正式范围**（§16）；默认 `shared`，按组织开通 |
| 7 | M1 充值 | 继续 **超管线下充组织池**；在线进池属 M4 |
...
> 对照结论：即使做完 M1–M5，相对 DogRouter 组织页仍偏弱的能力。  
...
成员或工作区管理员 **申请** 开放某模型 → 组织 admin（或平台指定审批人）**批准/驳回** → 通过后写入工作区白名单。  
**不做** 逐请求人工审批。
...
| M4b（轻量） | M4c（增强） |
| --- | --- |
| 组织填抬头；提交发票申请；超管线下开 | 申请可绑定 **账期账单**；状态机更完整；可对接开票服务商 API |
| 无税务文档库 | 组织可上传/下载 **合同、一般纳税人证明、开票记录 PDF** |
...
### B.9.3 M4d — 上游发票与组织账本勾兑

#### 目标

平台运营能把 **供应商账单/发票**（BasicRouter 等）与 **对组织收取的额度/金额**、付款、调账对齐，找出差额。
...
```text
组织侧：Σ customer_ledger / usage 折算金额（展示币种）
...
新组织开通或启用高风险能力前，走 **审查清单**（区域、数据驻留诉求、业务用例、是否对外服务等），通过后才放开对应策略开关。
...
- [ ] 未通过 onboarding 审查的组织可限制为「仅 default 工作区 + 低风险模型」  
- [ ] 审查记录可审计、可导出  

---

### B.9.5 M6b — 内容安全护栏（组织策略）

#### 目标

在组织/工作区级配置 **输入/输出护栏**：敏感词、PII 脱敏开关、第三方审核回调；命中可阻断或仅记日志。
...
平台超管（组织 admin 看本组织子集）对异常用量、突发 RPM、护栏命中、余额耗尽、失败率进行 **近实时** 观察与告警。
...
集团总部管多家子公司、或代理商给下游组织分额度。

#### 模型

```text
平台 → 机构 Institution → 组织 Customer → 工作区 Workspace
额度：平台 → 机构池 → 组织池 → 工作区池 → 调用扣工作区
```
...
| 项 | 设计 |
| --- | --- |
| 用户归属 | 仍挂在 **组织**；机构管理员通过「可管理的组织列表」授权 |
