...
现网组织只能由**平台超管**创建（`POST /api/customers`，指定 owner）。个人仍走 `/sign-up` 现有流程。

本期增加：**用户在注册时选择「组织」，自助开户**，成为该组织 owner。超管开户路径保留。
...
第 2 页：组织名称 + 邀请成员邮箱（可增删）。按钮：「返回」回第 1 页；「创建账户」一次提交：建用户 + 创建组织（当前用户为 owner）+ 写邀请。

### 2.3 OAuth / 微信（组织）

第 1 页点 OAuth / 微信：按现网先完成账号创建或登录，再进入第 2 页。第 2 页「创建账户」只补创建组织和邀请（用户已存在）。

第 1 页若走账号密码「下一步」，第 2 页「创建账户」才真正注册 + 开户。

### 2.4 邀请链接

`/sign-up?invite=...`（或现有邀请参数）只走**个人现有流程**，不进入第 2 页；注册后接受邀请加入已有组织，不新建组织。
...
1. 调用现有 `CreateCustomerWithOwner`：组织 + `slug=default` 工作区 + owner + `users.customer_id`  
2. `upstream_mode=shared`；**不**自动开通 BYOK  
3. 组织池额度 **0**  
4. 有邀请邮箱则写现有邀请记录（未注册者后续注册并接受；已有组织则邀请侧按现规拒绝）

### 3.3 组织（OAuth 已有用户）

用户已登录且 `customer_id` 为空：第 2 页提交只创建组织 + 邀请。已有 `customer_id` 则拒绝（一人一家组织）。
...
- 用户 ↔ 组织仍为一对一  
- 组织令牌只扣工作区池，不扣 `User.Quota`  
- 个人令牌仍走个人额度  
- 自助组织额度 0，需超管充值后再划拨工作区，工作区令牌才能调用  
- 不做：在线支付直充组织池、开户审批流、一人多家组织、把组织信息塞进个人单页表单、登录后再做独立引导向导  
...
- 自助组织：额度 0、`shared`、BYOK 关；邀请失败不回滚用户/组织
- 未做：超管组织列表区分来源（自助 / 超管）  
